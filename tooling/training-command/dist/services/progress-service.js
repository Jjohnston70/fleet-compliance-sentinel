...
class ProgressService {
    ...
    async recordModuleCompletion(enrollmentId, moduleId, score, timeSpentMinutes) {
    ...
    async getProgressSummary(enrollmentId) {
        ...
        const avgScore = enrollment.module_completions.length > 0
            ? enrollment.module_completions.reduce((sum, mc) => sum + (mc.score || 0), 0) / enrollment.module_completions.length
            : 0;
        const lastActivity = enrollment.module_completions.length > 0
            ...
            : null;
        const currentStreak = this.calculateStreak(enrollment.module_completions);
        return {
            enrollmentId,
    ...
    async getStudentProgressAcrossEnrollments(studentId) {
        const enrollments = await this.repo.listEnrollments({
            studentId,
            status: 'active',
        });
        const summaries = await Promise.all(enrollments.map((e) => this.getProgressSummary(e.id)));
        return summaries;
    }
    calculateStreak(completions) {
        if (completions.length === 0)
            return 0;
        const sortedCompletions = [...completions].sort((a, b) => b.completed_at.getTime() - a.completed_at.getTime());
        let streak = 1;
