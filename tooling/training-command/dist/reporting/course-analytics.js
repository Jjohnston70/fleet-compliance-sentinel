...
class CourseAnalytics {
    ...
    async getCourseAnalytics(courseId) {
        ...
        const avgProgress = Math.round(enrollments.reduce((sum, e) => sum + e.progress_pct, 0) /
            (enrollments.length || 1));
        const avgTimeSpent = Math.round(enrollments.reduce((sum, e) => sum + e.time_spent_minutes, 0) /
            (enrollments.length || 1) /
            60);
        ...
        for (const enrollment of enrollments) {
            for (const completion of enrollment.module_completions) {
                moduleCounts.set(completion.module_id, (moduleCounts.get(completion.module_id) || 0) + 1);
            }
        }
...
//# sourceMappingURL=course-analytics.js.map