  ...
  return [
    {
      name: 'list_courses',
      description:
        'List available training courses with optional filters by category or level',
      input_schema: {
        type: 'object',
        properties: {
          category: {
            type: 'string',
            description:
              'Filter by category (ai_fundamentals, data_strategy, automation, industry_specific, leadership)',
          },
          level: {
            type: 'string',
...
export function createToolHandlers(repo: IRepository): Record<string, ToolHandler> {
  const handlers = new APIHandlers(repo);

  return {
    list_courses: async (input) =>
      handlers.listCourses({
    ...
    enroll_student: async (input) =>
      handlers.enrollStudent(input.studentId, input.courseId),

    get_student_progress: async (input) =>
      handlers.getProgressSummary(input.enrollmentId),
    ...
    list_workshops: async (input) =>
      handlers.listWorkshops({ status: input.status }),

    register_for_workshop: async (input) =>
      handlers.registerForWorkshop(input.workshopId, input.studentId),

    get_student_dashboard: async (input) =>
      handlers.getStudentDashboard(input.studentId),

    issue_certificate: async (input) =>
...
export const TRAINING_COMMAND_TOOLS = createToolDefinitions();
