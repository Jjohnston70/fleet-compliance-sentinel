...
export async function getKPISummary(period = 'monthly', endDate) {
...
export async function getTrendData(dateFrom, dateTo, groupBy = 'daily') {
    let dateGroup = '';
    switch (groupBy) {
        case 'daily':
...
export async function getChannelBreakdown(dateFrom, dateTo) {
    const result = await query(`
    SELECT
      sr.channel,
      COALESCE(SUM(sr.total_revenue), 0) as revenue
