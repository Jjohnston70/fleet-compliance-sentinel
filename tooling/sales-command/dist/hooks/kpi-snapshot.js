...
export async function createKPISnapshot(date, period) {
    let dateFilter = '';
    const params = [date];
    switch (period) {
        case 'daily':
    ...
    for (const metric of metrics) {
        const metricResult = await query(`SELECT ${metric.query} as value FROM sales_records sr ${dateFilter}`, params);
        const currentValue = parseFloat(metricResult.rows[0].value) || 0;
        // Get previous value for comparison
        let previousValue = 0;
        const prevDate = calculatePreviousPeriod(date, period);
        if (prevDate) {
            let prevParams = [prevDate];
            let prevFilter = '';
            switch (period) {
                case 'daily':
            ...
            const prevResult = await query(`SELECT ${metric.query} as value FROM sales_records sr ${prevFilter}`, prevParams);
        ...
        await query(`INSERT INTO kpi_snapshots (date, period_type, metric_name, metric_value, previous_value, change_pct)
       VALUES ($1, $2, $3, $4, $5, $6)
       ON CONFLICT (date, period_type, metric_name) DO UPDATE SET 
       metric_value = $4, previous_value = $5, change_pct = $6`, [date, period, metric.name, currentValue, previousValue, changePct]);
    }
}
export async function dailyKPISnapshot() {
    const today = new Date().toISOString().split('T')[0];
    await createKPISnapshot(today, 'daily');
}
export async function weeklyKPISnapshot() {
    const today = new Date().toISOString().split('T')[0];
...
export async function quarterlyKPISnapshot() {
    const today = new Date().toISOString().split('T')[0];
    await createKPISnapshot(today, 'quarterly');
}
function calculatePreviousPeriod(date, period) {
