...
export async function generateMovingAverageForecast(
  ...
  const avgRevenue = monthlyData.reduce((sum: number, item: any) => sum + item.revenue, 0) / monthlyData.length;
...
export async function saveForecast(forecast: Forecast): Promise<void> {
  await query(
...
export async function getForecast(productId?: string, limit: number = 12): Promise<Forecast[]> {
  let whereClause = '1=1';
  const params: any[] = [];
  ...
  const result = await query(
    `SELECT * FROM sales_forecasts WHERE ${whereClause} ORDER BY period_start DESC LIMIT $${params.length + 1}`,
    [...params, limit]
  );
