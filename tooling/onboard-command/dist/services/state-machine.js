import { OnboardingStatus, QueueActionType, QueueItemStatus, } from '../data/schema.js';
export class OnboardingStateMachine {
    ...
    async evaluateState(requestId) {
        ...
        const completeOrSkipped = queueItems.filter((item) => item.status === QueueItemStatus.Complete || item.status === QueueItemStatus.Skipped).length;
    ...
    async transitionState(requestId, newStatus) {
