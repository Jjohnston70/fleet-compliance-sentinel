...
export class StandardRollbackEngine implements RollbackEngine {
  constructor(private repo: Repository, private auditService: AuditService) {}

  async rollbackRequest(requestId: string, reason: string): Promise<OnboardingRequest> {
    ...
    const reverseActions: ProvisioningQueueItem[] = [];
  ...
  private getReverseAction(item: ProvisioningQueueItem): ProvisioningQueueItem | null {
    // Map each action to its reverse
    const reverseMap: Record<QueueActionType, QueueActionType | null> = {
