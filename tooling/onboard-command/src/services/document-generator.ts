import { OnboardingRequest, DocumentType, GeneratedDocument } from '../data/schema.js';
...
export class StandardDocumentGenerator implements DocumentGenerator {
  ...
  renderTemplate(template: string, data: Record<string, any>): string {
    ...
    result = result.replace(/\{\{#(\w+)\}\}([\s\S]*?)\{\{\/\1\}\}/g, (match: string, key: string, content: string) => {
