...
export class DiscoveryService {
  ...
  private readonly fallbackTools: Record<string, ToolDefinition[]> = {
    'asset-command': [
      {
        name: 'list_assets',
        description:
          'List fleet assets with optional status and asset type filters, including pagination.',
        parameters: {
          type: 'object',
          properties: {
            status: {
  ...
  private resolveModuleToolSourcePath(moduleId: string): string | null {
  ...
  private readBalancedBraces(source: string, startIndex: number): string | null {
  ...
  private collectObjectRanges(source: string): Array<{ start: number; end: number }> {
  ...
  private extractQuotedProperty(objectLiteral: string, propertyName: string): string | null {
  ...
  private extractSchemaLiteral(objectLiteral: string): string | null {
    const schemaKeyMatch = /\b(parameters|input_schema)\s*:/.exec(objectLiteral);
    if (!schemaKeyMatch) return null;
  ...
  private normalizeSchema(rawSchema: unknown): ToolDefinition['parameters'] {
  ...
  private evaluateObjectLiteral(objectLiteral: string): unknown {
    try {
  ...
  private extractToolsFromSource(moduleId: string, source: string): ToolDefinition[] {
  ...
  private loadModuleTools(moduleId: string): ToolDefinition[] {
   ...
   */
  async rescan(): Promise<DiscoveryResult> {
