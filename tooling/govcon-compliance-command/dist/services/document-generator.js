 ...
 */
export async function generateDocx(title, content, options) {
    ...
    // Add footer
    if (options?.footer) {
        children.push(new Paragraph({ text: "", spacing: { before: 400 } }), new Paragraph({
            children: [
                new TextRun({
                    text: options.footer,
                    size: 18,
 ...
 */
export async function generatePdf(title, content, options) {
 ...
 */
export async function generateDocumentOutputs(title, slug, content, formats = ["docx", "pdf", "markdown"]) {
