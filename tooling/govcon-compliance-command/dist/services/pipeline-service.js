...
export class PipelineService {
    ...
    async recordPeriodMetrics(period, date, metrics) {
    ...
    async getMonthlyMetrics(year, month) {
        ...
        return (metrics.find((m) => m.date.getFullYear() === year && m.date.getMonth() === month - 1) || null);
    }
    async calculateLiveMetrics() {
        ...
        const bidOpps = opps.filter((o) => o.status === "bid" || o.status === "submitted");
        const total_bid_value = bidOpps.reduce((sum, o) => sum + (o.estimated_value || 0), 0);
        const won_opps = opps.filter((o) => o.status === "awarded");
        const total_won_value = won_opps.reduce((sum, o) => sum + (o.estimated_value || 0), 0);
    ...
    async getWinLossAnalysis(days = 180) {
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - days);
        const opps = (await this.repo.listOpportunities()).filter((o) => o.updated_at >= cutoffDate);
    ...
    async getAverageBidValue() {
        const opps = await this.repo.listOpportunities();
        const bidOpps = opps.filter((o) => o.status === "bid" || o.status === "submitted" || o.status === "awarded");
        if (bidOpps.length === 0)
            return 0;
