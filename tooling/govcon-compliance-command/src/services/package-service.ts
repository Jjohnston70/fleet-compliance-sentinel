...
import { CompanyRecord, CompliancePackage } from "../data/schemas.js";
...
export class CompliancePackageService {
  ...
  async generatePackage(
    ...
    try {
      ...
      const { rendered, unresolvedPlaceholders } = renderTemplate(
        templateContent,
        company
      );

      // Generate output documents
      const documents = await generateDocumentOutputs(
        pkgConfig.name,
        pkgConfig.slug,
        rendered,
        formats
  ...
  async generateAll(
    ...
    for (const pkgConfig of COMPLIANCE_PACKAGES) {
      try {
        const result = await this.generatePackage(
          companyId,
          pkgConfig.id,
          formats
        );
   ...
   */
  async getPackageStatus(companyId: string): Promise<
    Array<{
      packageNumber: number;
      packageName: string;
      status: string;
  ...
  > {
    ...
    return COMPLIANCE_PACKAGES.map((pkgConfig) => {
      const existing = packages.find(
        (p) => p.package_number === pkgConfig.id
      );
      return {
        packageNumber: pkgConfig.id,
        packageName: pkgConfig.name,
