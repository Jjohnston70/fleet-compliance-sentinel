...
export interface ComplianceAPI {
  listComplianceItems(filters?: { authority?: ComplianceItem["authority"]; status?: ComplianceItem["status"] }): Promise<ComplianceItem[]>;
  ...
  getComplianceAlerts(): Promise<{ critical: ComplianceItem[]; warning: ComplianceItem[]; upcoming: ComplianceItem[] }>;
  checkAuthorityCompliance(authority: ComplianceItem["authority"]): Promise<{ compliant: boolean; items: ComplianceItem[]; alerts: string[] }>;
}
