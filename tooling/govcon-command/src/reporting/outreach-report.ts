...
export class OutreachReport {
  ...
  /**
   * Generate outreach metrics report
   */
  async generateMetrics(): Promise<OutreachMetrics> {
    ...
    const averageContactsPerContact =
      contacts.length > 0
    ...
    const lastContactDate = contacts
      ...
      .sort(
        (a, b) =>
          (b.last_contacted?.getTime() || 0) -
          (a.last_contacted?.getTime() || 0)
      )[0]?.last_contacted || null;
   ...
   */
  async getHighEngagementContacts(
    minContactCount: number = 3
  ): Promise<OutreachContact[]> {
    const contacts = await this.outreachService.listContacts();
    return contacts
      .filter((c) => c.contact_count >= minContactCount)
      .sort((a, b) => b.contact_count - a.contact_count);
  }

  /**
   * Get cold contacts (not contacted recently)
   */
  async getColdContacts(daysSinceContact: number = 60): Promise<OutreachContact[]> {
    ...
    return contacts.filter(
      (c) => !c.last_contacted || c.last_contacted < cutoffDate
    );
  }

  /**
   * Get OSDBU contact summary
   */
  async getOSDBUSummary(): Promise<{
    totalOSDBU: number;
    byStatus: Record<OutreachContact["status"], number>;
    recentContacts: OutreachContact[];
  }> {
    const osdbuContacts = await this.outreachService.listContacts({
      osdbu: true,
    });

    const byStatus: Record<OutreachContact["status"], number> = {
      prospect: 0,
      warm: 0,
      active: 0,
      cold: 0,
    ...
    const recentContacts = osdbuContacts
      .filter((c) => c.last_contacted)
      .sort(
        (a, b) =>
          (b.last_contacted?.getTime() || 0) -
    ...
    return {
      totalOSDBU: osdbuContacts.length,
      byStatus,
      recentContacts,
    };
  }
