...
export class PipelineService {
  ...
  /**
   * Create or update pipeline metrics for a period
   */
  ...
  /**
   * Calculate current pipeline from live opportunity data
   */
  async calculateLiveMetrics(): Promise<{
    ...
    let opportunities_identified = opps.length;
    let bids_submitted = opps.filter((o) => o.status === "submitted").length;
    let wins = opps.filter((o) => o.status === "awarded").length;
    let losses = opps.filter((o) => o.status === "lost").length;
    let no_bids = opps.filter((o) => o.status === "no_bid").length;
