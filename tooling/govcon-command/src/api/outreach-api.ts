...
export function createOutreachAPI(service: OutreachService): OutreachAPI {
  return {
    async createContact(name, email, agency, title) {
      return service.createContact(name, email, agency, title);
    },

    async listContacts(filters) {
      return service.listContacts(filters);
    },
    ...
    async logOutreachActivity(contactId, activityType, subject) {
      return service.logActivity(contactId, activityType, subject);
    },

    async getOutreachHistory(contactId) {
      return service.getContactHistory(contactId);
    },

    async scheduleFollowUp(contactId, subject, followUpDate) {
      return service.scheduleFollowUp(contactId, subject, followUpDate);
    },
