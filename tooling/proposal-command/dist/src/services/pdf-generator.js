...
export class PDFGenerator {
     ...
     */
    static async generateDocx(proposal, template, clientName, clientCompany, lineItems, subtotal, tax, discount, total) {
        const children = [
            // Cover Page
     ...
     * Create cover page
     */
    static createCoverPage(proposal, clientName, clientCompany) {
     ...
     */
    static createTemplateContent(template, proposal) {
        const primaryColor = CONFIG.BRANDING.PRIMARY_COLOR.replace('#', '');
        return template.sections
            ...
            const content = new Paragraph({
                ...
                children: [
                    new TextRun(section.contentTemplate
     ...
     */
    static createPricingSection(lineItems, subtotal, tax, discount, total) {
        const primaryColor = CONFIG.BRANDING.PRIMARY_COLOR.replace('#', '');
        const secondaryColor = CONFIG.BRANDING.SECONDARY_COLOR.replace('#', '');
        const headerRow = new TableRow({
            children: ['Description', 'Qty', 'Unit Price', 'Total'].map(label => new TableCell({
                children: [
                    new Paragraph({
        ...
        const itemRows = lineItems.map(item => new TableRow({
            children: [
     ...
     */
    static createTermsSection(template, _proposal) {
        const primaryColor = CONFIG.BRANDING.PRIMARY_COLOR.replace('#', '');
        return [
            new Paragraph({
                heading: HeadingLevel.HEADING_2,
