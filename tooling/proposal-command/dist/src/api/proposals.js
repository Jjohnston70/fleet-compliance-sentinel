...
export async function createProposal(req, res) {
    ...
    try {
        const { clientName, clientCompany, clientEmail, projectTitle, projectDescription, serviceType, lineItems, validityDays = 30, } = req.body;
        ...
        const total = Array.isArray(lineItems)
            ...
            : 0;
        // Create proposal
        const proposal = await proposalService.createProposal(client.id, template.id, projectTitle, projectDescription || '', total, validityDays);
        // Update service type
        proposal.serviceType = serviceType;
        await repo.saveProposal(proposal);
        // Add line items
        if (Array.isArray(lineItems)) {
            await pricingService.addLineItems(proposal.id, lineItems.map((item, index) => ({
...
export async function updateProposal(req, res) {
    ...
    try {
        ...
        const updated = await proposalService.updateProposal(id, updates);
...
export async function sendProposal(req, res) {
    ...
    try {
        ...
        const summary = await pricingService.calculatePricingSummary(proposal.id);
        // Generate PDF
        const pdfBuffer = await PDFGenerator.generateDocx(proposal, template, client.contactName, client.companyName, lineItems, summary.subtotal, summary.taxAmount, summary.discountAmount, summary.total);
        // Create email payload
        const emailPayload = EmailService.createProposalEmailPayload(client.email, client.contactName, proposal.title, client.companyName, pdfBuffer, proposal.proposalNumber);
        // Send email
 ...
 */
export async function trackProposal(req, res) {
    try {
        const { id } = req.query;
        const proposal = await proposalService.getProposal(id);
        if (!proposal) {
        ...
        res.status(200).send(Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==', 'base64'));
    }
    catch (error) {
