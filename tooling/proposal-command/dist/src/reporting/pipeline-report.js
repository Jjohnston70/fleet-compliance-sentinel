...
export class PipelineReport {
    ...
    async generateReport() {
        ...
        proposals.forEach(p => {
            ...
            if (p.status === 'sent')
                metrics.conversionMetrics.sentCount++;
            if (p.status === 'accepted')
                metrics.conversionMetrics.acceptedCount++;
            if (p.status === 'declined')
                metrics.conversionMetrics.declinedCount++;
        });
        ...
        const closedProposals = proposals.filter(p => p.status === 'accepted' || p.status === 'declined');
     ...
     */
    async getHighValueProposals(threshold = 50000) {
        const proposals = await this.repository.listProposals();
        return proposals
