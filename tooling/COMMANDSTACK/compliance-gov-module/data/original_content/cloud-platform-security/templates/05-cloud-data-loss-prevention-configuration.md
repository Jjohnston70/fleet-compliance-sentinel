[COMPANY_LEGAL_NAME]
[CLOUD_PROVIDER] DATA LOSS PREVENTION CONFIGURATION STANDARD

Purpose

This [CLOUD_PROVIDER] Data Loss Prevention Configuration Standard establishes [COMPANY_LEGAL_NAME]'s requirements for implementing and managing [CLOUD_PLATFORM] Data Loss Prevention (DLP) to discover, classify, and protect sensitive data. This standard ensures sensitive data is identified and protected from unauthorized disclosure or loss.

Scope

This standard applies to all [COMPANY_NAME] data stored or processed in [CLOUD_PLATFORM_WORKSPACE] and [CLOUD_PLATFORM]. It applies to all personnel responsible for implementing or managing DLP controls.

Governance and Cross-Package References

This standard implements [COMPANY_ABBREVIATION] data protection requirements using [CLOUD_PLATFORM] DLP technology, supporting the no-PII-in-logs principle.

Universal [COMPANY_ABBREVIATION] Principles - DLP Implementation:
...
security-governance skill: Master governance document establishing data protection requirements
data-handling-privacy skill: Data classification system; DLP implements automated classification and protection
data-handling-privacy skill (anonymization): DLP de-identification techniques implement anonymization requirements
cloud-platform-security skill (workspace security): DLP integration with Workspace (Gmail, Drive, etc.)
...
DLP findings trigger remediation per internal-compliance skill (incident response)

For data classification, see data-handling-privacy skill.
For anonymization techniques, see data-handling-privacy skill (anonymization).
...
[CLOUD_PLATFORM_WORKSPACE] (Gmail, Drive) via DLP API
...
DLP for [CLOUD_PLATFORM_WORKSPACE]

Gmail DLP

Gmail DLP Rules
DLP rules configured in [CLOUD_PLATFORM_WORKSPACE] Admin Console
