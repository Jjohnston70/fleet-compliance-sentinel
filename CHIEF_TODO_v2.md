# CHIEF PLATFORM — IMPLEMENTATION TODO v2

**True North Data Strategies | Pipeline Punks**
**Project**: Chief Sentinel — Fleet & DOT Compliance SaaS
...
- Chief module: /app/chief/* pages and /api/chief/* routes
...
- ARCHITECTURE.md — Plain English system description: what Chief does,
  how data flows from CSV upload to Postgres to Chief pages to Penny,
...
> Record current state: pipelinepunks.com loads, Chief pages load, Penny responds.
...
```
I am building a SOC 2 Type I compliant SaaS product called Chief — a fleet
...
```
You are hardening the Chief platform for production. Execute tasks in order.
...
Create /components/chief/ChiefErrorBoundary.tsx:
- Catches errors from Chief data fetches
...
Wrap these pages with ChiefErrorBoundary:
/chief, /chief/assets, /chief/compliance, /chief/suspense,
/chief/employees, /chief/invoices, /chief/fmcsa, /chief/alerts,
/chief/import
...
Create /app/api/chief/cron-health/route.ts (admin protected):
...
- ChiefErrorBoundary wrapping all nine pages
- migrations/001_cron_log.sql committed
- /api/chief/cron-health route working
...
- [x] `ChiefErrorBoundary.tsx` wrapping all nine Chief pages ✓
- [x] `migrations/001_cron_log.sql` committed ✓
- [x] `/api/chief/cron-health` route implemented ✓
...
- `components/chief/ChiefErrorBoundary.tsx`
- `migrations/001_cron_log.sql`
- `app/api/chief/cron-health/route.ts`
- `RAILWAY_CONFIG.md`
- `UPTIME_SETUP.md`
...
- `ChiefErrorBoundary.tsx` → `soc2-evidence/monitoring/error-boundary-code.md`
...
Apply requireChiefOrg to EVERY route in /api/chief/* that reads or
writes data. List every route you update in the output.

TASK 2 — PARAMETERIZED QUERY AUDIT
Review every SQL query in /lib/chief-data.ts and /api/chief/* routes.
...
Create /api/chief/[collection]/[id]/restore route (admin only):
...
Create /api/chief/import/rollback route:
- Accepts { batchId: string }
- Verifies batch belongs to requesting org (requireChiefOrg)
...
Create /lib/chief-validators.ts with:
...
OUTPUT REQUIRED:
- List every API route updated with requireChiefOrg
...
- [x] `/lib/chief-auth.ts` with `requireChiefOrg` and role variant
- [x] Every `/api/chief/*` route applies `requireChiefOrg` (12/12 routes verified)
- [x] Parameterized query audit complete — reviewed scope safe; no string-built SQL in `/api/chief/*`
- [x] `chief_records` table has `deleted_at` column (soft delete)
- [x] All SELECT queries include `WHERE deleted_at IS NULL`
- [x] `chief_records` table has `import_batch_id` column
...
- [x] `/lib/chief-validators.ts` with four validators (drivers, assets, permits, employees)
- [x] Import save returns 422 with field-level errors on invalid data
- [x] Build passes

### Outstanding Blockers (must fix before Phase 3)
- [x] Invoice module: `requireChiefOrgWithRole` + `org_id` column + org-scoped queries
- [x] Cron secret: `crypto.timingSafeEqual()` comparison in alerts run route

### Open High Findings
- [x] Add `/chief(.*)` and `/api/chief/(.*)` to middleware route matcher
- [x] Remove unused `_request` parameter from `requireChiefOrg` or use it
...
- [x] Error boundaries still wrapping Chief pages
...
- `chief-auth.ts` → `soc2-evidence/access-control/chief-auth-code.md`
...
- Every GET /api/chief/* — log data.read with collection and record count
- Every POST /api/chief/* — log data.write with collection and new record ID
...
- Chief page error boundary triggered: diagnose steps
...
- All existing Chief pages must continue to work after migration
...
This converts Chief from a single-client tool to a multi-tenant SaaS.
...
ALTER TABLE chief_records ADD COLUMN IF NOT EXISTS org_id TEXT NOT NULL DEFAULT 'org_default';
...
Update every query in /lib/chief-data.ts to filter by org_id.
...
Call ensureOrgProvisioned in Chief layout after auth verification.
...
Create ChiefTrialBanner showing days remaining in trial.
Create expired trial blocking screen when isActive is false.
Add plan gate check to Chief layout.

TASK 5 — ONBOARDING FLOW
Create /app/chief/onboarding page:
...
- Redirects to /chief/import after completion

Redirect to /chief/onboarding if onboarding_complete is false
and this is their first Chief login.
...
- Confirm ensureOrgProvisioned called on Chief layout
...
```
You are connecting Penny to org-specific Chief data.
...
- Target URLs: /chief/*, /api/chief/*, /api/penny/*
...
- How to set up CODEOWNERS for security-sensitive files:
  /lib/chief-auth.ts, /lib/audit-logger.ts, vercel.json,
  all migration files
...
- [ ] requireChiefOrg blocking unauthorized requests
...
```
You are documenting business continuity procedures for Chief.
...
- P2 High: Chief pages failing, Penny down, import broken
...
| Neon | Database | All Chief records | SOC 2 Type II | ... |
...
2. ACCESS_CONTROL_POLICY.md
   ...
   - Org isolation enforcement (reference chief-auth.ts)
...
_True North Data Strategies LLC | Chief Platform Implementation Plan v2_
