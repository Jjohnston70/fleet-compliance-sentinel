...
export interface ChiefAlertItem {
...
export interface ChiefEmailPayload {
  to: string;
  subject: string;
  html: string;
  items: ChiefAlertItem[];
}

export interface ChiefAlertRunSummary {
  runAt: string;
...
function dueLine(item: ChiefAlertItem): string {
...
function buildEmailHtml(ownerEmail: string, items: ChiefAlertItem[], orgName: string): string {
  const rows = items
    .map(
      (item) => `
      <tr>
  ...
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>Chief Compliance Reminder</title></head>
        ...
        <tr>
          <td style="padding:24px">
            ...
            <p style="margin:20px 0 0;font-size:13px;color:#6b7280">
              Log in to the Chief compliance portal to review and action these items.
            </p>
          </td>
        </tr>
        <tr>
...
async function sendEmail(payload: ChiefEmailPayload, apiKey: string): Promise<{ ok: boolean; error?: string }> {
  const fromAddress = process.env.CHIEF_ALERT_FROM_EMAIL || 'compliance@examplefleetco.com';
  try {
...
export async function runChiefAlertSweep(
  ...
  const byOwner = new Map<string, ChiefAlertItem[]>();
  ...
  const emails: ChiefEmailPayload[] = [];
  for (const [ownerEmail, items] of byOwner.entries()) {
    ...
    const subject = isManager
      ? `[Chief] Compliance summary: ${items.length} item${items.length !== 1 ? 's' : ''} — ${windowParts.join(', ') || 'review needed'}`
      : `[Chief] Compliance reminder: ${items.length} item${items.length !== 1 ? 's' : ''} need attention`;
  ...
  const summary: ChiefAlertRunSummary = {
...
export function previewChiefAlerts(suspenseItems: ChiefSuspenseRecord[], referenceDate?: Date): {
  ...
  const byOwner: Record<string, ChiefAlertItem[]> = {};
  for (const item of alertItems) {
