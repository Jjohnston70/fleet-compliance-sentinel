/**
 * Async data layer for Fleet-Compliance module — reads from Neon Postgres.
 *
 * Replaces fleet-compliance-demo-data.ts which read from the (now-empty) generated file.
...
export type FleetComplianceAssetCategory = string;

export interface FleetComplianceAssetRecord {
  assetId: string;
  sourceRef: string;
  name: string;
  category: FleetComplianceAssetCategory;
  status: string;
  location: string;
  assignedTo: string;
...
export interface FleetCompliancePermitRecord {
  recordId: string;
  name: string;
  templateId: string;
  cadence: string;
...
export interface FleetComplianceDriverComplianceRecord {
  personId: string;
  employeeId: string;
  driverName: string;
  licenseState: string;
...
export interface FleetComplianceSuspenseRecord {
...
export interface FleetComplianceActivityLogRecord {
  activityId: string;
  assetId: string;
  timestamp: string;
  actionType: string;
...
export interface FleetComplianceMaintenanceEventRecord {
...
export interface FleetComplianceImportCollectionStat {
  name: string;
  count: number;
}
...
async function loadCollection(collection: string, orgId: string): Promise<RawRow[]> {
  try {
    ...
    const rows = await sql`
      SELECT data FROM fleet_compliance_records
...
function transformTanks(raw: RawRow[]): FleetComplianceAssetRecord[] {
  return raw.map((r) => ({
    assetId: s(r['Tank ID']),
...
function transformVehicles(raw: RawRow[]): FleetComplianceAssetRecord[] {
  return raw.map((r) => ({
...
function transformDrivers(raw: RawRow[]): FleetComplianceDriverComplianceRecord[] {
  return raw.map((r) => ({
...
function transformEmployeesToDrivers(raw: RawRow[]): FleetComplianceDriverComplianceRecord[] {
  return raw.map((r) => ({
...
function transformPermits(raw: RawRow[]): FleetCompliancePermitRecord[] {
  return raw.map((r, i) => {
...
function transformActivityLogs(raw: RawRow[]): FleetComplianceActivityLogRecord[] {
  return raw.map((r, i) => ({
    activityId: `act-${i + 1}`,
    assetId: s(r['Asset ID']),
...
function transformMaintenanceEvents(raw: RawRow[]): FleetComplianceMaintenanceEventRecord[] {
  return raw.map((r) => ({
...
function transformMaintenanceSchedule(raw: RawRow[]): FleetComplianceMaintenanceEventRecord[] {
  return raw.map((r, i) => ({
...
function transformEmployees(raw: RawRow[]): FleetComplianceEmployeeRecord[] {
  return raw.map((r) => ({
    employeeId: s(r['Employee ID']),
...
function generateSuspenseFromDrivers(drivers: FleetComplianceDriverComplianceRecord[]): FleetComplianceSuspenseRecord[] {
...
function generateSuspenseFromPermits(permits: FleetCompliancePermitRecord[]): FleetComplianceSuspenseRecord[] {
  const items: FleetComplianceSuspenseRecord[] = [];
...
function generateSuspenseFromAssets(assets: FleetComplianceAssetRecord[]): FleetComplianceSuspenseRecord[] {
  const items: FleetComplianceSuspenseRecord[] = [];
...
export interface FleetComplianceData {
  assets: FleetComplianceAssetRecord[];
...
): FleetComplianceDriverComplianceRecord[] {
...
export function filterSuspenseItems(
  items: FleetComplianceSuspenseRecord[],
...
export function findAsset(assets: FleetComplianceAssetRecord[], assetId: string) {
  return assets.find((a) => a.assetId === assetId) || null;
}

export function findDriver(drivers: FleetComplianceDriverComplianceRecord[], personId: string) {
  return drivers.find((d) => d.personId === personId) || null;
}

export function findPermit(permits: FleetCompliancePermitRecord[], recordId: string) {
  return permits.find((p) => p.recordId === recordId) || null;
}

export function findSuspenseItem(items: FleetComplianceSuspenseRecord[], suspenseItemId: string) {
  return items.find((i) => i.suspenseItemId === suspenseItemId) || null;
}

export function findMaintenanceEvent(events: FleetComplianceMaintenanceEventRecord[], id: string) {
  return events.find((e) => e.maintenanceEventId === id) || null;
}

export function findActivityLog(logs: FleetComplianceActivityLogRecord[], id: string) {
  return logs.find((l) => l.activityId === id) || null;
}

// ── Relation functions ────────────────────────────────────────────────────

export function getAssetActivity(logs: FleetComplianceActivityLogRecord[], assetId: string) {
  return logs.filter((l) => l.assetId === assetId).slice(0, 8);
}

export function getAssetMaintenance(events: FleetComplianceMaintenanceEventRecord[], assetId: string) {
  return events.filter((e) => e.assetId === assetId).slice(0, 8);
}

export function getAssetSuspense(items: FleetComplianceSuspenseRecord[], assetId: string) {
  return items.filter((i) => i.sourceId === assetId);
}

export function getDriverSuspense(items: FleetComplianceSuspenseRecord[], personId: string) {
  return items.filter((i) => i.sourceId === personId);
}

export function getPermitSuspense(items: FleetComplianceSuspenseRecord[], recordId: string) {
  return items.filter((i) => i.sourceId === recordId);
}

export function getSuspenseRelatedRecord(
  item: FleetComplianceSuspenseRecord,
  data: FleetComplianceData
) {
...
export function getAssetStats(assets: FleetComplianceAssetRecord[]) {
  return {
    total: assets.length,
...
export function getComplianceStats(drivers: FleetComplianceDriverComplianceRecord[], permits: FleetCompliancePermitRecord[]) {
  return {
    ...
    requirements: fleetCompliancePermitCadence.length,
  };
}

export function getSuspenseStats(items: FleetComplianceSuspenseRecord[]) {
  return {
...
export async function getImportStats(orgId: string): Promise<{ generatedAt: string; collections: FleetComplianceImportCollectionStat[] }> {
  try {
    const sql = getSQL();
    const rows = await sql`
...
export function getAssetSourceQuality(asset: FleetComplianceAssetRecord): SourceQuality {
...
export function getDriverSourceQuality(driver: FleetComplianceDriverComplianceRecord): SourceQuality {
...
export function getPermitSourceQuality(permit: FleetCompliancePermitRecord): SourceQuality {
...
export const fleetCompliancePermitCadence = [
...
export const fleetComplianceResourceLinks: FleetComplianceResourceLink[] = [
  { href: '/fleet-compliance/import', label: 'Document Intake', note: 'Use Import Review for manuals, permits, certificates, and support file workflows.' },
...
const DOC_LINKS_BY_CATEGORY: Record<string, FleetComplianceDocumentLink[]> = {
  vehicle: [
    { label: 'DOT Inspection Certificate', note: 'Annual 49 CFR 396.17 inspection cert', href: '/fleet-compliance/import' },
    { label: 'Vapor Test Certificate', note: 'Annual vapor recovery test cert', href: '/fleet-compliance/import' },
    { label: 'Registration & Insurance', note: 'Current registration and insurance binder', href: '/fleet-compliance/import' },
    { label: 'Driver Vehicle Inspection Reports (DVIR)', note: 'Pre/post trip inspection records', href: '/fleet-compliance/import' },
  ],
  tank: [
    { label: 'UST Permit', note: 'Colorado CDPHE underground/aboveground storage tank permit', href: '/fleet-compliance/import' },
    { label: 'Leak Test Certificate', note: 'Annual leak detection test results', href: '/fleet-compliance/import' },
    { label: 'ATG Calibration Record', note: 'Annual automatic tank gauge calibration', href: '/fleet-compliance/import' },
    { label: 'Spill & Overfill Equipment Inspection', note: 'Annual inspection documentation', href: '/fleet-compliance/import' },
  ],
  trailer: [
    { label: 'Annual Inspection Certificate', note: '49 CFR 393 trailer inspection cert', href: '/fleet-compliance/import' },
    { label: 'Registration', note: 'Current trailer registration', href: '/fleet-compliance/import' },
  ],
};

const DEFAULT_ASSET_DOCS: FleetComplianceDocumentLink[] = [
  { label: 'Maintenance Records', note: 'Service history and work orders', href: '/fleet-compliance/import' },
  { label: 'Purchase Documentation', note: 'Purchase agreement or invoice', href: '/fleet-compliance/import' },
];

const DRIVER_DOCS: FleetComplianceDocumentLink[] = [
  { label: 'CDL Copy', note: 'Current commercial driver license', href: '/fleet-compliance/import' },
  { label: 'Medical Certificate', note: 'DOT physical medical examiner certificate', href: '/fleet-compliance/import' },
  { label: 'Motor Vehicle Record (MVR)', note: 'Annual MVR pull from state DMV', href: '/fleet-compliance/import' },
  { label: 'Drug & Alcohol Test Records', note: 'Pre-employment and random test results', href: '/fleet-compliance/import' },
  { label: 'Clearinghouse Query Results', note: 'FMCSA Drug & Alcohol Clearinghouse record', href: '/fleet-compliance/import' },
  { label: 'Hazmat Training Certificate', note: '3-year hazmat training certification (if applicable)', href: '/fleet-compliance/import' },
];

const PERMIT_DOCS: Record<string, FleetComplianceDocumentLink[]> = {
  state_hazmat: [
    { label: 'CO Hazmat Registration Certificate', note: 'Issued by Colorado CDPHE', href: '/fleet-compliance/import' },
    { label: 'MCS-90 Endorsement', note: 'Motor carrier insurance endorsement', href: '/fleet-compliance/import' },
  ],
  federal_hazmat: [
    { label: 'PHMSA Hazmat Registration Certificate', note: 'PHMSA-issued federal hazmat registration', href: '/fleet-compliance/import' },
    { label: 'Insurance Certificate', note: 'Proof of financial responsibility', href: '/fleet-compliance/import' },
  ],
  ucr: [{ label: 'UCR Confirmation', note: 'Annual Unified Carrier Registration receipt', href: '/fleet-compliance/import' }],
  operating_authority_mc150: [
    { label: 'MCS-150 Filing Confirmation', note: 'FMCSA biennial update acknowledgment', href: '/fleet-compliance/import' },
    { label: 'Operating Authority Certificate', note: 'MC number certificate of authority', href: '/fleet-compliance/import' },
  ],
  ifta: [
    { label: 'IFTA License', note: 'Current quarter IFTA license', href: '/fleet-compliance/import' },
    { label: 'Fuel Purchase Records', note: 'Supporting fuel receipts by state', href: '/fleet-compliance/import' },
  ],
  irp: [{ label: 'IRP Cab Cards', note: 'Current apportioned registration cab cards for each vehicle', href: '/fleet-compliance/import' }],
};

const DEFAULT_PERMIT_DOCS: FleetComplianceDocumentLink[] = [
  { label: 'Permit Certificate', note: 'Issued permit or license document', href: '/fleet-compliance/import' },
  { label: 'Renewal Correspondence', note: 'Agency renewal notices and confirmations', href: '/fleet-compliance/import' },
];

export function getAssetDocuments(category: string): FleetComplianceDocumentLink[] {
  return [...(DOC_LINKS_BY_CATEGORY[category] || DEFAULT_ASSET_DOCS)];
}

export function getDriverDocuments(): FleetComplianceDocumentLink[] {
  return DRIVER_DOCS;
}

export function getPermitDocuments(templateId: string): FleetComplianceDocumentLink[] {
