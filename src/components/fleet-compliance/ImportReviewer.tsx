...
export default function ImportReviewer({ orgId }: { orgId?: string }) {
  ...
  async function handleUpload() {
    ...
    try {
      const res = await fetch('/api/fleet-compliance/import/parse', {
        method: 'POST',
        body: formData,
      });
  ...
  async function saveToDatabase() {
    ...
    try {
      const res = await fetch('/api/fleet-compliance/import/save', {
        method: 'POST',
  ...
  async function rollbackImport(batchId: string) {
    ...
    try {
      const res = await fetch('/api/fleet-compliance/import/rollback', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  function downloadApproved() {
    ...
    a.href = url;
    a.download = 'fleet-compliance-import-approved.json';
    a.click();
  ...
  return (
    <div>
      ...
      <div className="fleet-compliance-list-card">
        ...
        <div className="fleet-compliance-filter-grid" style={{ marginBottom: '1rem' }}>
        ...
        <label className="fleet-compliance-table-note" style={{ display: 'block', marginBottom: '0.75rem' }}>
          <input
            type="checkbox"
      ...
      {view.state === 'saved' && (
        <div className="fleet-compliance-list-card" style={{ marginTop: '1.5rem', borderLeft: '4px solid #16a34a' }}>
        ...
        <>
          ...
          <div className="fleet-compliance-stats fleet-compliance-stats-compact" style={{ marginTop: '1.5rem' }}>
            <article className="fleet-compliance-stat-card">
              <p className="fleet-compliance-stat-label">Sheets Parsed</p>
              <p className="fleet-compliance-stat-value">{view.result.sheetsParsed}</p>
            </article>
            <article className="fleet-compliance-stat-card">
              <p className="fleet-compliance-stat-label">Total Rows</p>
              <p className="fleet-compliance-stat-value">{view.result.totalRows}</p>
            </article>
            <article className="fleet-compliance-stat-card">
              <p className="fleet-compliance-stat-label">Pass</p>
              <p className="fleet-compliance-stat-value" style={{ color: '#16a34a' }}>{view.result.totalPass}</p>
            </article>
            <article className="fleet-compliance-stat-card">
              <p className="fleet-compliance-stat-label">Issues</p>
              <p className="fleet-compliance-stat-value" style={{ color: view.result.totalWarn > 0 ? '#d97706' : undefined }}>
                {view.result.totalWarn}
              </p>
            </article>
            <article className="fleet-compliance-stat-card">
              <p className="fleet-compliance-stat-label">Approved</p>
              <p className="fleet-compliance-stat-value" style={{ color: '#16a34a' }}>{totalApproved}</p>
            </article>
          </div>
              ...
              return (
                <div key={sheet.collection}>
                  ...
                  {sheet.missingHeaders.length > 0 && (
                    <div className="fleet-compliance-info-banner" style={{ marginTop: '0.75rem' }}>
                  ...
                  {sheet.totalRows === 0 ? (
                    <div className="fleet-compliance-empty-state" style={{ marginTop: '0.75rem' }}>
                        ...
                        <tbody>
                          {sheet.rows.map((row) => {
                            ...
                            return (
                              ...
                              >
                                <td className="fleet-compliance-table-note">{row.rowIndex}</td>
                                <td>
                                  <button
                                ...
                                {sheet.expectedHeaders.map((h) => (
                                  <td key={h} className={!row.data[h] ? 'fleet-compliance-table-note' : undefined}>
                                    {row.data[h] || '\u2014'}
                                  </td>
                                ))}
                                <td>
      ...
      {view.state === 'saving' && (
        <div className="fleet-compliance-empty-state" style={{ marginTop: '1.5rem' }}>
