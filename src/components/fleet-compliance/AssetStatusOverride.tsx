...
export default function AssetStatusOverride({ assetId, initialStatus, statuses }: Props) {
  ...
  if (editing) {
    return (
      <dd style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <button className="fleet-compliance-icon-btn" onClick={handleSave} type="button">Save</button>
        <button className="fleet-compliance-icon-btn" onClick={() => setEditing(false)} type="button">Cancel</button>
      </dd>
    );
  }
  ...
  return (
    <dd style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
      <span className={`fleet-compliance-pill fleet-compliance-pill-${status.replace(/[^a-z0-9]+/g, '-').toLowerCase()}`}>
        {status}
      </span>
      {overridden && (
        <span className="fleet-compliance-table-note" style={{ color: 'var(--teal)' }}>
          (overridden this session)
        </span>
      )}
      <button className="fleet-compliance-icon-btn" onClick={() => setEditing(true)} type="button">
        Change
      </button>
      {overridden && (
        <button className="fleet-compliance-icon-btn" onClick={handleReset} type="button">
          Reset
        </button>
      )}
    </dd>
