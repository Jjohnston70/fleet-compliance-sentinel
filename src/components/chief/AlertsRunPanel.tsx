...
import type { ChiefAlertRunSummary } from '@/lib/chief-alert-engine';

type RunState =
  | { state: 'idle' }
  | { state: 'running' }
  | { state: 'done'; summary: ChiefAlertRunSummary }
...
export default function AlertsRunPanel({ resendConfigured }: { resendConfigured: boolean }) {
  ...
  async function trigger(dryRun: boolean) {
    ...
    try {
      const res = await fetch('/api/chief/alerts/trigger', {
        method: 'POST',
      ...
      setRun({ state: 'done', summary: data as ChiefAlertRunSummary });
    } catch (err: unknown) {
    ...
    <div className="chief-list-card" style={{ marginTop: '1.5rem' }}>
      ...
      {run.state === 'done' && (
        <div style={{ marginTop: '1.25rem' }}>
          <p className="chief-eyebrow" style={{ marginBottom: '0.5rem' }}>
          ...
          <dl className="chief-kv-list">
            ...
            <div>
              <dt>Emails sent</dt>
              <dd>{run.summary.dryRun ? <span className="chief-table-note">dry run — none sent</span> : run.summary.emailsSent}</dd>
            </div>
            ...
            <div><dt>Run at</dt><dd className="chief-table-note">{new Date(run.summary.runAt).toLocaleString()}</dd></div>
          </dl>
