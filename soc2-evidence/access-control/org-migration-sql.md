...
- Added org indexes (conditional by table existence):
  ...
  - `idx_invoices_org_id`
  - `idx_fleet_compliance_records_org_id`
- Created:
  - `organizations`
  - `subscriptions`
- Added:
...
```text
tables=fleet_compliance_records,invoices,organizations,subscriptions
org_id_columns=fleet_compliance_records,invoices
indexes=idx_fleet_compliance_records_org_id,idx_invoices_org_id,idx_subscriptions_org_id_created_at
counts={"fleet_compliance_records_count":81,"invoices_count":0,"organizations_count":0,"subscriptions_count":0}
```

Notes:
- This environment currently has `fleet_compliance_records` and `invoices` as active operational tables.
