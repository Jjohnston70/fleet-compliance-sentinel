...
Validated org-level data isolation for Fleet-Compliance multi-tenant scoping using two Clerk organizations and org-scoped `fleet_compliance_records` queries.

## Clerk Test Organizations

- Org A: `org_3BRjuemrfCkz36K31xEJhx2nUV9` (`Fleet-Compliance Isolation Org A 1774458999176`)
- Org B: `org_3BRjucb99fBnsia9IqDNquFkB9z` (`Fleet-Compliance Isolation Org B 1774458999176`)

## Procedure

1. Created two Clerk orgs via Clerk Admin API.
2. Called `ensureOrgProvisioned` for both org IDs.
3. Inserted org-scoped records into `fleet_compliance_records`:
   - Org A: asset `A-CLERK-001`
   - Org B: asset `B-CLERK-001`
4. Loaded data through `loadFleet-ComplianceData(orgId)` for each org.
